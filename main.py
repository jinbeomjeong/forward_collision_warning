...
vehicle_can_ch_1 = AdasCANCommunication(dbc_filename='../resource/ADAS_can_protocol.dbc')
radar_filter = DBSCAN(eps=0.05, min_samples=3, n_jobs=4)


def radar_com():
    while True:
        forward_radar.read_point_clouds()
...
while True:
    if forward_radar.get_n_point_clouds():
        radar_points = forward_radar.get_point_clouds()
